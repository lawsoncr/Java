...
public class DoubleLinkedSeq implements Cloneable
{
    ...
    public static void main(String[]args)
    {
        ...
        for (int i = 10; i >= 1; i--)
        {
           head = new DoubleNode(i, head);
        }
        DoubleNode cursor = DoubleNode.listSearch(head, 7);
        System.out.println(DoubleNode.listLength(cursor)); 
    ...
    {
        // code for addBefore
        DoubleNode dN = new DoubleNode(element);
        if (manyNodes == 0)
        {           
            head = dN;
            tail = dN;
    ...
    public void addAll(DoubleLinkedSeq other) throws NullPointerException
    {
        ...
        for (DoubleNode curs = other.head; curs != null; curs = curs.getLink())
        {
            temp.addAfter(curs.getData());
        }
    ...
    public DoubleLinkedSeq clone() throws RuntimeException
    {
        ...
        links = cloneHelp(head, cursor, precursor, head, tail);
        result.head = links[0];
        result.tail = links[1];
     ...
     * @return result asdf
     */
    public static DoubleNode[] cloneHelp(DoubleNode element, 
